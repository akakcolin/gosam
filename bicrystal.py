...
import numpy
from numpy import dot, identity, inner, zeros, arccos
...
class Bicrystal(OrthorhombicPbcModel):
    ...
    def generate_atoms(self, z_margin=0.):
        ...
        print("Number of atoms in bicrystal: {}".format(len(self.atoms)))
        self.print_boundary_angle()

    def print_boundary_angle(self):
        def calc_angle(v1, v2):
            # avoid the math domain error
            tmpdata =  dot(v1, v2) / sqrt(inner(v1,v1) * inner(v2,v2))
            if(tmpdata >1.00):
                return acos(1.0)
            else:
                return acos(tmpdata)
...
class BicrystalOptions:
    ...
    def parse_sigma_and_find_theta(self, sigma_arg):
        ...
        print("-------> theta = {0} deg".format(degrees(theta)))
        self.sigma = sigma
        self.theta = theta
        self.m = m
        self.n = n
    ...
    def find_dim(self, min_dim):
        print("-------> min. dim.  [A]:", min_dim[0], min_dim[1], min_dim[2], sep=", ")
...
def parse_args():
    ...
    print("-------> rotation axis: [{} {} {}]".format(*opts.axis))
...
def main():
    ...
    print("CSL cell with z || [{} {} {}]".format(*opts.plane))
    ...
    if opts.remove_dist2:
        ...
        for aa in a_atoms, b_atoms:
            print("Removing atoms where {0}-{1} distance is < {2} ...".format(
    ...
    if opts.edge:
        ...
        print("edge: {0} atoms is removed".format(len(sel)))
